...
(function (global, factory) {
  ...
  const variationPlacements = /* #__PURE__ */basePlacements.reduce((acc, placement) => {
    return acc.concat([`${placement}-${start}`, `${placement}-${end}`]);
  }, []);
  const placements = /* #__PURE__ */[].concat(basePlacements, [auto]).reduce((acc, placement) => {
  ...
  */
  ...
  function computeAutoPlacement(state, options) {
    ...
    const placements$1 = variation ? flipVariations ? variationPlacements : variationPlacements.filter((placement) => {
      return getVariation(placement) === variation;
    }) : basePlacements;
    let allowedPlacements = placements$1.filter((placement) => {
  ...
  function flip(_ref) {
    ...
    const placements = [preferredPlacement].concat(fallbackPlacements).reduce((acc, placement) => {
    ...
    if (makeFallbackChecks) {
      ...
      const _loop = function _loop(_i) {
        const fittingPlacement = placements.find((placement) => {
          ...
          if (checks) {
            return checks.slice(0, _i).every((check) => {
              return check;
            });
          }
        });
  ...
  function orderModifiers(modifiers) {
    ...
    return modifierPhases.reduce((acc, phase) => {
      return acc.concat(orderedModifiers.filter((modifier) => {
    ...
    return function createPopper(reference, popper, options) {
      ...
      var instance = {
        ...
        setOptions: function setOptions(options) {
          ...
          state.orderedModifiers = orderedModifiers.filter((m) => {
            return m.enabled;
        ...
        forceUpdate: function forceUpdate() {
          ...
          state.orderedModifiers.forEach((modifier) => {
            return state.modifiersData[modifier.name] = { ...modifier.data };
          });
