...
1. Pass the `selectedTabId` as a prop to the `Tabs`, the specified tab should be selected if possible
  (otherwise the first tab is selected).data-cy="tab-content"
1. The `Tabs` should show the content of the selected tab (add an attribute `` for testing).
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://Artem66.github.io/react_tabs/) and add it to the PR description.
