...
1. Save the `selectedTabIdId` in the `App` (the first `tab` is selected by default);
...
1. Pass the `selectedTabIdId` as a prop to the `Tabs`, the specified tab should be selected if possible
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://partnersinbahamas.github.io/react_tabs/) and add it to the PR description.
